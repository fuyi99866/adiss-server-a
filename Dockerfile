...
#copy go app
COPY --from=builder /go/src/adiss-server-a/gvadmin ./
COPY --from=builder /go/src/adiss-server-a/db.db ./
COPY --from=builder /go/src/adiss-server-a/config.yaml ./
COPY --from=builder /go/src/adiss-server-a/resource ./resource
